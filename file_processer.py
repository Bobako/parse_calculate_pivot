...
def write_xlsx(file_name,array,sheetname = "Sheet"):
	file = openpyxl.Workbook()
	#file.create_sheet(sheetname)
	#sheet = file[sheetname]
	sheet = file.active
	sheetname = file.sheetnames[0]
	c_names = list(array[0].keys())
	for i in range(len(c_names)):
		sheet.cell(row = 1, column = i+1, value = str(c_names[i]))
	file.save(file_name)

	for element in array:
		add_row_xlsx(file_name,sheetname,element)
...
def add_row_xlsx(file_name,sheetname,dict_):		#converts dict to a table row and adds it to xlsx file
	...
	for i in range(1,len(keys)+1):
		sheet.cell(row = l, column = i, value = (literals[i-1]))

	file.save(file_name)

def add_sheet(file_name,sheetname):					#if there is no sheet with name sheetname in a xlsx book, adds it and returns True, else returns False
	file = openpyxl.load_workbook(file_name)
